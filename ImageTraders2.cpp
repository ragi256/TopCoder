...
public:
	int maxOwners(vector <string> price) {
	    int n=price.size(),i,j,k,l,ans=0;
	    can[1][0][0] = true;
	    REP(i, (1<<n)) REP(j,n) REP(k,10) if(can[i][j][k]) REP(l,n) if(!(i&(i<<l)) && price[j][l] - '0' >= k) can[i+(1<<l)][l][price[j][l]-'0'] = true;
	    REP(i,(1<<n)){
		int cnt = 0;
		REP(j,n) if(i&(1<<j)) cnt++;
		REP(j,n) REP(k,10) if(can[i][j][k]) ans = max(ans,cnt);
	    }
	    return ans;
	}
};
