...
void debug(int* dp){
	...
	if(dp[i]==-1) continue;
	if(dp[i]==12) cout << bitset<15>(i) << ":" << dp[i] << endl;
    }
}

class ImageTraders {
int rec(bitset<15> status, int &owner, int &now_price, int &size){
    unsigned long st = status.to_ulong();
//    if(dp[st]!=-1) return dp[st];
    if(dp[owner][now_price][st]!=-1) return dp[owner][now_price][st];
    int ma=1;
    for(int i=0; i<size; i++){
	if(status[i]) continue;
	if(PRICE[owner][i] >= now_price){
	    int buyer = i;
	    bitset<15> next = status;
	    next.set(buyer);
	    ma = max(1+rec(next, buyer, PRICE[owner][buyer], size),ma);
	}
    }
    return dp[owner][now_price][st] = ma;
//    return dp[st] = ma;
}
...
public:
	...
	int maxOwners(vector <string> price) {
		...
		PRICE[i].resize(price.size());
		for(int j=0; j<price.size(); j++)
		    PRICE[i][j] = price[i][j] - '0';
	    }
//	    debug(dp);
	    int size = price.size(), owner=0, start_price=0;
	    memset(dp, -1, sizeof(dp));
	    return rec(owned, owner, start_price, size);
	}
};
